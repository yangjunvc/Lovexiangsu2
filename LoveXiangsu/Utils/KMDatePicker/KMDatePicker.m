...
#define kDefaultMinLimitedDate @"1970-01-01 00:00:00" // modified by Wolfgang
#define kDefaultMaxLimitedDate @"2060-12-31 23:59:59" // modified by Wolfgang
#define kMonthCountOfEveryYear 12
#define kHourCountOfEveryDay 24
#define kMinuteCountOfEveryHour 60
// new appended by Wolfgang
#define kSecondCountOfEveryMinute 60
...
- (void)setGreatThanMinDate:(NSDate *)date
{
    NSDate * newDate = nil;
    switch (_datePickerStyle) {
        ...
        case KMDatePickerStyleYearMonth:
        {
            ...
            break;
        case KMDatePickerStyleYearMonthDayHourMinuteSecond:
        {
            NSDateComponents * components = [[NSDateComponents alloc]init];
            components.second = 1;
            newDate = [[NSCalendar currentCalendar]
            ...
            break;
        case KMDatePickerStyleHourMinuteSecond:
        {
            NSDateComponents * components = [[NSDateComponents alloc]init];
            components.second = 1;
            newDate = [[NSCalendar currentCalendar]
            ...
            break;
        case KMDatePickerStyleMonthDay:
        {
            NSDateComponents * components = [[NSDateComponents alloc]init];
            components.day = 1;
            newDate = [[NSCalendar currentCalendar]
    ...
    _minLimitedDate = newDate;
}

- (void)setLessThanMaxDate:(NSDate *)date
{
    NSDate * newDate = nil;
    switch (_datePickerStyle) {
        case KMDatePickerStyleYearMonthDayHourMinute:
...
- (void)loadData {
    ...
    // new appended by Wolfgang
    //秒
    mArrSecond = [[NSMutableArray alloc] initWithCapacity:kSecondCountOfEveryMinute];
    for (NSInteger i=0; i<kSecondCountOfEveryMinute; i++) {
        [mArrSecond addObject:[NSString stringWithFormat:@"%02ld", (long)i]];
    }
    secondIndex = [datePickerDateScrollTo.second integerValue];
}
    ...
    switch (_datePickerStyle) {
        ...
        case KMDatePickerStyleYearMonthDayHourMinuteSecond: {
            arrIndex = @[
                         ...
                         [NSNumber numberWithInteger:hourIndex],
                         [NSNumber numberWithInteger:minuteIndex],
                         [NSNumber numberWithInteger:secondIndex]
                         ];
            break;
        }
        case KMDatePickerStyleHourMinuteSecond: {
...
- (BOOL)validatedDate:(NSDate *)date {
    ...
    NSString *minDateStr = [NSString stringWithFormat:@"%@-%@-%@ %@:%@:%@",
                            ...
                            datePickerDateMinLimited.second
                            ];
...
- (UIColor *)monthRowTextColor:(NSInteger)row {
    ...
    NSString *dateStr = [NSString stringWithFormat:@"%@-%@-01 00:00:00",// modified by Wolfgang
                         mArrYear[yearIndex],
                         mArrMonth[row]
                         ];
    NSDate *date = [DateHelper dateFromString:dateStr withFormat:nil];
    
    NSString *minDateStr = [NSString stringWithFormat:@"%@-%@-01 00:00:00",// modified by Wolfgang
                         datePickerDateMinLimited.year,
                         datePickerDateMinLimited.month
                         ];
...
- (UIColor *)dayRowTextColor:(NSInteger)row {
    UIColor *color = kRowNormalStatusColor;
    NSString *dateStr = [NSString stringWithFormat:@"%@-%@-%@ 00:00:00",// modified by Wolfgang
                         mArrYear[yearIndex],
    ...
    NSString *minDateStr = [NSString stringWithFormat:@"%@-%@-%@ 00:00:00",// modified by Wolfgang
                            datePickerDateMinLimited.year,
                            datePickerDateMinLimited.month,
                            datePickerDateMinLimited.day
                            ];
...
- (UIColor *)hourRowTextColor:(NSInteger)row {
    UIColor *color = kRowNormalStatusColor;
    NSString *dateStr = [NSString stringWithFormat:@"%@-%@-%@ %@:00:00",// modified by Wolfgang
                         mArrYear[yearIndex],
                         mArrMonth[monthIndex],
                         mArrDay[dayIndex],
                         mArrHour[row]
    ...
    NSString *minDateStr = [NSString stringWithFormat:@"%@-%@-%@ %@:00:00",// modified by Wolfgang
                            datePickerDateMinLimited.year,
                            datePickerDateMinLimited.month,
                            datePickerDateMinLimited.day,
                            datePickerDateMinLimited.hour
        ...
        case KMDatePickerStyleYearMonthDayHourMinuteSecond: {
            switch (component) {
                ...
                case 4:
                    ...
                    break;
                case 5:
                    numberOfRows = kSecondCountOfEveryMinute;
                    break;
            }
            break;
        }
...
- (CGFloat)pickerView:(UIPickerView *)pickerView widthForComponent:(NSInteger)component {
    ...
    switch (_datePickerStyle) {
        ...
        // new appended by Wolfgang
        case KMDatePickerStyleYearMonth: {
            widthOfAverage = (kWidthOfTotal - 20.0 - 15.0) / 2;
            switch (component) {
                case 0:
                    width = widthOfAverage + 20.0;
                    [self addUnitLabel:@"年" withPointX:width/2 + 24.0];
        ...
        case KMDatePickerStyleYearMonthDayHourMinuteSecond: {
            //规则一：平均宽度 = （总共宽度 - 年份相比多两个数字的宽度 - 分钟会是右对齐导致需偏移宽度来显示「分」这个文字） / 6等份
            widthOfAverage = (kWidthOfTotal - 20.0 - 28.0) / 6;
            switch (component) {
                case 0:
                    width = widthOfAverage + 20.0;
                    //规则二：单位标签的 X 坐标位置 = 列的水平居中 X 坐标 ＋ 偏移量
                    [self addUnitLabel:@"年" withPointX:width/2 + 25.0];
                    break;
                case 1:
                    width = widthOfAverage;
                    [self addUnitLabel:@"月" withPointX:(widthOfAverage + 20.0) + width/2 + 19.0];
                    break;
                case 2:
                    width = widthOfAverage-2.0;
                    [self addUnitLabel:@"日" withPointX:(2*widthOfAverage + 20.0) + width/2 + 22.0];
                    break;
                case 3:
                    width = widthOfAverage-2.0;
                    ...
                    break;
                case 4:
                    width = widthOfAverage-5.0;
                    [self addUnitLabel:@"分" withPointX:(4*widthOfAverage + 20.0) + width/2 + 30.0];
                    break;
                case 5:
                    width = widthOfAverage-2.0;
                    [self addUnitLabel:@"秒" withPointX:(5*widthOfAverage + 20.0) + width/2 + 30.0];
                    break;
            }
            break;
        }
        case KMDatePickerStyleHourMinuteSecond: {
            ...
            switch (component) {
                ...
                case 2:
                    width = widthOfAverage;
                    [self addUnitLabel:@"秒" withPointX:2*widthOfAverage + width/2 + 20.0];
                    break;
            }
            break;
        }
        case KMDatePickerStyleMonthDay: {
            widthOfAverage = (kWidthOfTotal - 15.0) / 2;
            switch (component) {
                case 0:
                    width = widthOfAverage;
                    [self addUnitLabel:@"月" withPointX:width/2 + 24.0];
                    break;
                case 1:
                    width = widthOfAverage;
                    [self addUnitLabel:@"日" withPointX:(widthOfAverage + 20.0) + width/2 + 12.0];
                    break;
            }
            break;
        }
...
- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component {
    ...
    NSString *dateStr = [NSString stringWithFormat:@"%@-%@-%@ %@:%@:%@", // modified by Wolfgang
                         mArrYear[yearIndex],
                         mArrMonth[monthIndex],
                         mArrDay[dayIndex],
                         mArrHour[hourIndex],
                         mArrMinute[minuteIndex],
                         mArrSecond[secondIndex] // modified by Wolfgang
                         ];
...
//        datePickerDateScrollTo = datePickerDateScrollToTemp;
        ...
        hourIndex = [datePickerDateScrollTo.hour integerValue];
        minuteIndex = [datePickerDateScrollTo.minute integerValue];
        secondIndex = [datePickerDateScrollTo.second integerValue]; // new appended by Wolfgang
        [self scrollToDateIndexPosition];
        [self reloadDayArray];
        [pickerView reloadAllComponents];
    }
