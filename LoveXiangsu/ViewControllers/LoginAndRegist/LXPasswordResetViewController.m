//
//  LXPasswordResetViewController.m
//  LoveXiangsu
//
...
@interface LXPasswordResetViewController ()
{
    UIView *frameView;
}
...
@implementation LXPasswordResetViewController
...
//    self.lxLeftBtnImg.layer.borderColor = [[UIColor colorWithRed:123.0/255.0 green:212.0/255.0 blue:254.0/255.0 alpha:1.0] CGColor];
    ...
    [self.lxTitleLabel setTitle:@"密码重置" forState:UIControlStateNormal];
...
//    self.lxTitleLabel.layer.borderColor = [[UIColor colorWithRed:123.0/255.0 green:212.0/255.0 blue:254.0/255.0 alpha:1.0] CGColor];
    ...
    [self.lxRightBtnTitle setTitle:@"重置密码" forState:UIControlStateNormal];
    self.lxRightBtnTitle.frame = CGRectMake(0, 7, 70, 30);
...
- (void)finishBtnClick:(id)sender{
    ...
    if ([VertifyInputFormat isEmpty:self.passwordField.text]) {
        NSLog(@"提示：请输入新密码...");
        [self showTips:@"请输入新密码..." delay:2 anim:YES];
        return;
    }
    
    [self asyncConnectPasswordReset];
}
...
-(void)asyncConnectGetValidCode{
    ...
    LXPwdResetSendValidCodeRequest * getValid = [[LXPwdResetSendValidCodeRequest alloc]init];//初始化网络请求
...
-(void)asyncConnectPasswordReset{
    [self showHUD:@"请稍候..." anim:YES];
    LXPasswordResetRequest * pwdReset = [[LXPasswordResetRequest alloc]init];//初始化网络请求
    
    pwdReset.delegate = self;//代理设置
    pwdReset.tag = 1001;//类型标识
    
    NSMutableDictionary * paraDic = [NSMutableDictionary dictionary];//入参字典
    [paraDic setObject:self.phoneNumField.text forKey:@"phone"];
    [paraDic setObject:self.codeField.text forKey:@"validate"];
    [paraDic setObject:self.passwordField.text forKey:@"newpassword"];
    [pwdReset setCustomRequestParams:paraDic];//设置入参
    
    [[LXNetManager sharedInstance] addRequest:pwdReset];//发送网络请求
}
...
-(void)requestResult:(NSError*) error withData:(id)responseObject responseData:(LXBaseRequest*)requestData
{
    if (requestData.tag == 1000) {
        ...
        if (![VertifyInputFormat isEmpty:requestData.successMsg]) {
            ...
            [self showSuccess:@"重置成功，请重新登录。" delay:2 anim:YES];
        }
