//
//  LXMyCalledStoresViewController.m
//  LoveXiangsu
//
...
@interface LXMyCalledStoresViewController ()
...
@implementation LXMyCalledStoresViewController
...
- (void)viewDidLoad {
    ...
    [self asyncConnectGetMyCalledStores];
}
...
- (void)putNavigationBar
{
    ...
    [self.lxTitleLabel setTitle:@"我拨打过的店铺" forState:UIControlStateNormal];
...
{
    static NSString *storeCellIdentifier = @"CalledCell";
...
-(void)asyncConnectGetMyCalledStores{
    [self showHUD:@"正在加载..." anim:YES];
    LXMyCalledStoresRequest * request = [[LXMyCalledStoresRequest alloc]init];//初始化网络请求
...
-(void)requestResult:(NSError*) error withData:(id)responseObject responseData:(LXBaseRequest*)requestData
{
    if (requestData.tag == 1000) {
        ...
        if ([responseObject isKindOfClass:[NSArray class]]) {
            ...
            [self showError:@"获取我拨打过的店铺列表失败" delay:2 anim:YES];
        }
    }
