//
//  LXStoreAddServiceViewController.m
//  LoveXiangsu
//
...
@interface LXStoreAddServiceViewController ()<UIAlertViewDelegate>
...
@implementation LXStoreAddServiceViewController
...
- (void)putNavigationBar
{
    ...
    [self.lxTitleLabel setTitle:@"添加服务" forState:UIControlStateNormal];
    ...
    [self.lxRightBtnTitle addTarget:self action:@selector(lxAddServiceBtnClick:) forControlEvents:UIControlEventTouchUpInside];
...
- (void)lxAddServiceBtnClick:(id)sender{
    NSLog(@"添加服务按钮 Clicked");
    
    [self asyncConnectStoreAddService];
}
...
-(void)asyncConnectStoreAddService{
    [self showHUD:@"正在提交..." anim:YES];
    LXStoreAddServiceRequest * request = [[LXStoreAddServiceRequest alloc]init];//初始化网络请求
