//
//  LXStoreServiceListViewController.m
//  LoveXiangsu
//
...
#import "LXStoreAddServiceViewController.h"
...
@interface LXStoreServiceListViewController ()
...
@implementation LXStoreServiceListViewController
...
- (void)viewDidLoad {
    ...
    [self asyncConnectGetServiceList];
...
{
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(refreshServiceList) name:k_Noti_refreshServiceList object:nil];
}
...
{
    [[NSNotificationCenter defaultCenter] removeObserver:self name:k_Noti_refreshServiceList object:nil];
}
...
- (void)putNavigationBar
{
    ...
    [self.lxTitleLabel setTitle:@"服务列表" forState:UIControlStateNormal];
    ...
    [self.lxRightBtn2Img addTarget:self action:@selector(newServiceBtnClick:) forControlEvents:UIControlEventTouchUpInside];
...
- (void)putElements
{
    ...
    //    if (IOS8_OR_LATER) {
    //        CGFloat height = MARGIN * 2 + MARGIN * 2 + MARGIN * 2 + ServiceImgHeight;
...
- (void)newServiceBtnClick:(id)sender{
    NSLog(@"发新服务按钮 Clicked");
    if (![VertifyInputFormat isEmpty:[LXUserDefaultManager getUserNonce]]) {
        dispatch_async(dispatch_get_main_queue(), ^{
            LXStoreAddServiceViewController * vc = [[LXStoreAddServiceViewController alloc]init];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    LXStoreSimpleCell * cell = [tableView dequeueReusableCellWithIdentifier:serviceCellIdentifier];
    if (cell == nil) {
        cell = [[LXStoreSimpleCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:serviceCellIdentifier];
    ...
    LXServiceInfoResponseModel *model = self.tableArray[indexPath.row];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        NSLog(@"服务ID：%@ 被选择", cell.relative_id);
        LXStoreModifyServiceViewController * vc = [[LXStoreModifyServiceViewController alloc]init];
        vc.service_id = cell.relative_id;
        vc.order_id = cell.order_id;
        UINavigationController *nav = [[UINavigationController alloc]initWithRootViewController:vc];
        nav.navigationBarHidden = NO;
        [self presentViewController:nav animated:YES completion:nil];
    });
...
-(void)asyncConnectGetServiceList{
    [self showHUD:@"正在加载..." anim:YES];
    LXServiceListByStoreIdRequest * request = [[LXServiceListByStoreIdRequest alloc]init];//初始化网络请求
...
-(void)requestResult:(NSError*) error withData:(id)responseObject responseData:(LXBaseRequest*)requestData
{
    if (requestData.tag == 1000) {
        ...
        if ([responseObject isKindOfClass:[NSArray class]]) {
            ...
            [self showError:@"获取店铺服务列表失败" delay:2 anim:YES];
        }
    }
